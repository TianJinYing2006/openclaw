...
public class BfrTools implements AiTool {
    ...
    public Object calculateBfr(
        ...
        if (!bfrService.isConfigured()) {
            return InformationToolSupport.unavailable("体脂率服务");
        }
        try {
            BfrInfo result = bfrService.calculateBfr(age, height, weight, sex);
            return result.toString();
        } catch (RuntimeException exception) {
            return InformationToolSupport.failed("体脂率计算", exception);
        }
    }
}
