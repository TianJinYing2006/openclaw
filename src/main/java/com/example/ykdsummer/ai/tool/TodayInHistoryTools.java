...
public class TodayInHistoryTools implements AiTool {
    ...
    public Object queryTodayInHistory(
            @ToolParam(required = false, description = "月份，1-12，不传则查今天")
            Integer month,
            @ToolParam(required = false, description = "日期，1-31，不传则查今天")
            Integer day
    ) {
        if (!todayInHistoryService.isConfigured()) {
            return InformationToolSupport.unavailable("历史事件查询服务");
        }
        try {
            ...
            return InformationToolSupport.failed("历史事件查询", exception);
        }
    }
}
