 ...
 * <p>三极缓存（P1.5 持久化）：
 * <ol>
 *   <li>进程内 Caffeine（30 分钟）——会话内最快</li>
 *   <li>MySQL 持久缓存 {@link QueryAnalysisStore}——跨进程复用同一分析结果，
 *       固化检索词，消除 LLM 分析漂移导致的评测噪声（实测同配置 top-5 在
 *       40%~61% 摆动）与 token 重复消耗</li>
 *   <li>LLM 分析（greedy temperature=0）——未命中时兜底生成</li>
 * </ol>
 * 持久层可通过 {@code app.fashion.rag.analysis-cache.enabled} 关闭（默认开）。
 */
@Component
public class QueryAnalyzer {
    ...
    /** 持久缓存层；容器缺该 bean（如按需裁剪）时为 null，仅内存缓存。 */
    private final QueryAnalysisStore analysisStore;
    ...
    public QueryAnalyzer(AgentLlmCaller llmCaller,
                         @Autowired(required = false) QueryAnalysisStore analysisStore,
                         @Value("${app.fashion.rag.analysis-cache.enabled:true}") boolean persistentCacheEnabled) {
    ...
    public AnalyzedQuery analyze(String userInput, String profileContext) {
        ...
        // 持久缓存：跨进程复用同一分析结果，固化检索词（P1.5）
        if (persistentCacheEnabled && analysisStore != null) {
            AnalyzedQuery stored = analysisStore.get(cacheKey);
            if (stored != null) {
                analysisCache.put(cacheKey, stored);
                log.info("QueryAnalyzer persistent cache hit for input: {}", truncate(userInput));
                return stored;
            }
        }
        ...
        if (result != null) {
            ...
            if (persistentCacheEnabled && analysisStore != null) {
                analysisStore.put(cacheKey, result);
            }
        }
        return result;
    }
    ...
    private AnalyzedQuery analyzeUncachedInternal(String userInput, String contextualInput) {
        ...
        // maxTokens=600：原 200 不够输出 hypotheticalOutfit（80-150 中文字约 300-500
        // tokens），实测会截断 JSON 字段值导致解析失败回退 fallback（关键词拼接的
        // 弱 HyDE）。提到 600 给 LLM 足够空间输出完整 HyDE 描述。
