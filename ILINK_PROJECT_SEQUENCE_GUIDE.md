...
4. Java 根据消息内容分到文字、看图、语音转写或生图通道；
5. 得到结果后，SDK 使用 `sendmessage` 把文字或图片发回微信。
...
sequenceDiagram
    ...
    participant 图片队列 as 图片独立队列
    participant AI as 文字或图片模型
...
- 同一用户的文字消息保持顺序；
- `生图：`进入独立图片线程，不阻塞后面的文字；
- 图片可能比后发送的文字更晚回复，这是正常的并发结果。
...
sequenceDiagram
    ...
    participant 主服务 as iLink 主服务
    participant 分流 as 消息分流服务
    participant 图片队列 as 独立图片线程池
    participant 生图服务 as AI图片生成服务
    participant 模型 as 图片模型
    ...
    分流-->>主服务: 确认为图片任务
    主服务->>图片队列: 提交独立任务
    Note over 主服务,图片队列: 主服务立即返回，文字队列可继续工作
    图片队列->>分流: 正式解析并移除“生图：”前缀
    分流->>生图服务: 传入用户编号和画面描述
    生图服务->>模型: 请求生成一张PNG图片
    Note over 生图服务,模型: 最多等待6分钟，不重试
    alt 成功返回Base64图片
        模型-->>生图服务: 返回Base64数据
        生图服务->>生图服务: 解码为图片字节并检查大小
        生图服务-->>分流: 返回图片回复对象
        分流-->>图片队列: 返回图片字节
        图片队列->>SDK: 调用图片发送方法
        SDK->>SDK: 加密图片并申请上传地址
...
图片线程池有 2 个工作线程，所以最多同时处理 2 个生图任务。图片任务之间的完成顺序不保证与发送顺序相同。
...
| 理解消息类型处理 | [ILinkReplyService.java](src/main/java/com/example/ykdsummer/bot/service/ILinkReplyService.java) | 已完成当前版本 | 文字、图片、文件、语音转写、短视频和固定命令均已有明确分流；文件上传后进入文档模式，后续文字先判断分析、修改或歧义确认 |
