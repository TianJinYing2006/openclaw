...
- **用户可见文本质量（LLM-as-judge）**：见 `docs/agent-judge-results.md`。
  - v2 主实验（30 条去重）：完整管线 **33.3% vs 裸基线 63.3%**（落后）；
  - 定位到主因是**用户可见文本夹带内部工件**（`outfit_XXX`、试穿编号标记）；
  - 清理后用当前代码复测 **v4：60.0% vs 40.0%（翻盘领先）**；
  - v3 曾尝试改提示词（23.3%）无正收益已回滚；
  - 结论：**管线整体价值已被证实**；但**评审子链路（critic/trend/coordinator）的额外质量收益仍未证明**，
    故 `critic-loop` 保持默认关闭（延迟 3.5~3.9x、Critic 打回 0/20）。
