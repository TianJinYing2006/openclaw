# 穿搭方案 096：阿kong - 米白针织Polo米白阔腿裤_正面裁切

## 基本信息
- 编号：096
- 来源：小红书/@阿kong
- 原始图片：96_阿kong_米白针织Polo米白阔腿裤_正面裁切.webp
- 单品数量：3
- 图片质量：GOOD
- 识别到的服装数：3

## 整套搭配概览
- 组成：上衣(米白色条纹Polo衫)，下装(米白色宽松休闲裤)，鞋子(白色运动鞋)
- 整体风格：学院/街头/通勤
- 适合季节：夏季
- 适合场合：日常/通勤
- 配色方案：米白色 + 米白色 + 白色
- 整体正式度：3.0/5

## 单品1：米白色条纹Polo衫
- 类别：上衣（Polo衫）
- 适用性别：UNISEX
- 颜色：主色=米白色, 辅色=白色, 色调=NEUTRAL, 近似色值=#E8E3D8
- 外观：图案=条纹, 版型=常规, 轮廓=H型, 长度=常规长度
- 材质：未知成分针织面料
- 面料属性：thickness=LIGHT, stretch=MEDIUM, drape=MEDIUM, sheen=MATTE, breathability=MEDIUM, texture=RIBBED
- 结构：collar=POLO_COLLAR, sleeve=SHORT_SLEEVE, closure=PULLOVER
- 风格：街头/学院/通勤
- 季节：夏季
- 场合：日常/通勤
- 正式度：3/5
- 特征：条纹结构

## 单品2：米白色宽松休闲裤
- 类别：下装（休闲裤）
- 适用性别：UNISEX
- 颜色：主色=米白色, 辅色=白色, 色调=NEUTRAL, 近似色值=#E8E3D8
- 外观：图案=纯色, 版型=宽松, 轮廓=阔腿, 长度=全长
- 材质：化纤混纺
- 面料属性：thickness=LIGHT, stretch=LOW, drape=MEDIUM, sheen=MATTE, breathability=MEDIUM, texture=WOVEN
- 结构：waist=UNKNOWN, legShape=WIDE_LEG
- 风格：街头/学院/通勤
- 季节：夏季
- 场合：日常/通勤
- 正式度：3/5
- 特征：无明显特征

## 单品3：白色运动鞋
- 类别：鞋子（运动鞋）
- 适用性别：UNISEX
- 颜色：主色=白色, 色调=NEUTRAL, 近似色值=#F2F2EF
- 外观：图案=纯色, 版型=常规, 轮廓=挺括轮廓, 长度=常规长度
- 材质：化纤混纺
- 面料属性：thickness=MEDIUM, stretch=NONE, drape=NONE, sheen=MATTE, breathability=LOW, texture=UNKNOWN
- 结构：shoeCut=LOW_TOP
- 风格：街头/学院/通勤
- 季节：夏季
- 场合：日常/通勤
- 正式度：3/5
- 特征：无明显特征
